...
function regressionFixture(failGetFor) {
  return makeGithub({
    ...
    commentsByIssue: { 2661: [{ body: '## EPIC_RESCOPE' }], 2662: [{ body: '## CONSULTANT_CLOSEOUT rubric 8/10' }] },
    failGetFor,
  });
}
...
test('AC3+AC6: closing the #2661-shaped Epic is blocked, reopened, and fails the run', async () => {
  ...
  const r = await guard.run({ github, context: ctx(2661), core });
...
test('AC5: a blocked close emits a phase0-complete-no-phase1 incident', async () => {
  ...
  const { core } = makeCore();
  delete process.env.PHASE0_GATE_BYPASS;
  await guard.run({ github, context: ctx(2661), core });
...
test('AC3: a compliant Epic (Phase-1 present) is not blocked', async () => {
  const { github } = makeGithub({
    ...
    commentsByIssue: { 500: [{ body: 'EPIC_RESCOPE' }], 501: [{ body: 'CONSULTANT_CLOSEOUT 9/10' }] },
  });
  const { core, calls } = makeCore();
  const r = await guard.run({ github, context: ctx(500), core });
