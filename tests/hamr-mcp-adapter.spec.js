// hamr-mcp-adapter tests — Refs #3043, #3796
...
// default-OFF gate (MEGINGJORD_MCP_ADAPTER_ENABLED).
'use strict';
...
test('main() returns adapter-disabled-default-off when MEGINGJORD_MCP_ADAPTER_ENABLED is unset', async () => {
  const origEnabled = process.env.MEGINGJORD_MCP_ADAPTER_ENABLED;
  const origQuiet = process.env.MEGINGJORD_QUIET_RESOLVER;
  delete process.env.MEGINGJORD_MCP_ADAPTER_ENABLED;
  process.env.MEGINGJORD_QUIET_RESOLVER = '1'; // suppress stderr advisory
  try {
    const result = await ADAPTER.main();
    expect(result.served).toBe(false);
    expect(result.reason).toBe('adapter-disabled-default-off');
    expect(result.flag).toBe('MEGINGJORD_MCP_ADAPTER_ENABLED');
  } finally {
    if (origEnabled === undefined) delete process.env.MEGINGJORD_MCP_ADAPTER_ENABLED;
    else process.env.MEGINGJORD_MCP_ADAPTER_ENABLED = origEnabled;
    if (origQuiet === undefined) delete process.env.MEGINGJORD_QUIET_RESOLVER;
    else process.env.MEGINGJORD_QUIET_RESOLVER = origQuiet;
  }
});

test('main() attempts MCP server startup when MEGINGJORD_MCP_ADAPTER_ENABLED=1', async () => {
  const origEnabled = process.env.MEGINGJORD_MCP_ADAPTER_ENABLED;
  const origQuiet = process.env.MEGINGJORD_QUIET_RESOLVER;
  process.env.MEGINGJORD_MCP_ADAPTER_ENABLED = '1';
  process.env.MEGINGJORD_QUIET_RESOLVER = '1';
  try {
    // With the gate enabled, main() will try to require @modelcontextprotocol/sdk
    // and start a server. We expect it to either succeed or throw from the SDK
    // requiring stdio transport (no actual stdin in test). Either way the gate passed.
    const result = await ADAPTER.main().catch((err) => ({ threw: true, message: err.message }));
    // If it threw, it got past the gate (the gate returns, it does not throw)
    if (result.threw) {
      expect(result.message).toBeTruthy();
    } else {
      // If it returned, it should report served: true (server started)
