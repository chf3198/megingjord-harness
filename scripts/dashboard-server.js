...
async function handleApi(req, res) {
  ...
  if (requestUrl === '/api/anneal/queue') return require('../dashboard/api/anneal-queue-handlers').handleAnnealQueue(req, res); if (requestUrl === '/api/goal-coverage') return require('../dashboard/api/goal-coverage-handlers').handleGoalCoverage(req, res); if (requestUrl === '/api/hamr-coverage') return require('../dashboard/api/hamr-coverage-handlers').handleHamrCoverage(req, res); if (requestUrl === '/api/hamr-offload') { const { hamrOffloadPayload } = require('../dashboard/api/hamr-offload-handlers'); return jsonRes(res, 200, hamrOffloadPayload()); } if (requestUrl === '/api/merge-evidence-stats') return require('../dashboard/api/merge-evidence-handlers').handleMergeEvidenceStats(req, res); if (requestUrl === '/api/wiki-health') { return jsonRes(res, 200, getWikiHealth()); } if (requestUrl === '/api/projects-v2/in-flight') return require('../dashboard/api/projects-v2-handlers').handleProjectsV2InFlight(req, res); if (requestUrl === '/api/fleet-cost') return require('../dashboard/api/fleet-cost-handlers').handleFleetCost(req, res);
