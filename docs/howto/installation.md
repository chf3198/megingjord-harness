...
1. Copy the workspace adapter files:

   ```bash
   cp -r /path/to/megingjord-harness/.github/copilot-instructions.md .github/
   cp /path/to/megingjord-harness/CLAUDE.md .
   cp /path/to/megingjord-harness/AGENTS.md .
   cp -r /path/to/megingjord-harness/.claude .
   ```

2. Customize the workspace adapter to reflect the project's purpose.
3. Commit the workspace files to the project repo.

The global layer is already live. No further setup is required for Copilot,
Claude Code, or Codex to pick up the harness skills and governance hooks.
...
Deploy once → shared by all projects     Unique per project, committed to git
```

## Adding a second project

Run the project initialization step in the new repo. The global layer is
already in place — `npm run deploy:apply` is idempotent and safe to re-run
to pick up harness updates.

## Workspace overrides

A workspace file can override any global behavior:
...
Local wins on conflict (per provider-neutral governance contract).
...
All projects that share this machine automatically get the update.

## Workspace wiki

Each project can maintain its own workspace wiki at `wiki/` (source) that
compiles into `~/.copilot/wiki/`. Run `npm run wiki:ingest` (from the
harness repo after copying updated source) to refresh.
